// TODO:
// 1) column and value sorting for inserts
// 2) cross-check with the XMLs to see if anything missing
// 3) include all entities across all folders
// 4) test database insert
// 5) explore in database
// 6) proof of concept - simple DB query
// 7) database lineage
// 8) SVG - ERD
...
object Prototype4 extends App {
  ...
  val key_name = "mdsid" // the code below is supposed to be generic, suitable for many XML repositories, not just OBIEE. 'mdsid' key name here is the only bit that is OBIEE-specific.

  // ===================================================================================================================
  // === Entity ========================================================================================================
  case class Entity(node: Node, parentNode: Option[Node], rootNode: Option[Node]) {

    def entityType: String = (if (rootNode.isEmpty) "" else rootNode.get.label + "_") + node.label // add root node label in front of the child node's label

    def entityAttributes: Map[String,String] =
      node.attributes.asAttrMap ++ // node attributes
      Map("node_text" -> ( node.child collect {case Text(txt) if txt.trim != "" => txt.trim.replace("\n","    ").replace("'","''")} mkString(", ") )) ++ // node text (between node tags)
    ...
    def insertSql: String = {
      val sortedEntityAttributes: List[(String, String)] = entityAttributes.toList sortWith ((a, b) => Entities.attributeSorter((a,b)) )
      val sortedNames   = sortedEntityAttributes map {case(a,_) => a}
      val sortedValues  = sortedEntityAttributes map {case(_,a) => a}

      s"INSERT INTO $entityType (${sortedNames mkString(", ")}) VALUES (${sortedValues map (av => if (av == "N/A") "NULL" else s"'$av'") mkString(", ")})"
    }
  ...
  case class Entities(entities: Map[String, List[Entity]]) {
    ...
    def entityDDLs: String = {
      ...
      val ddl = attributeValuesAnalysed map {
        ...
        }.toList sortWith ( (a,b) => Entities.attributeSorter((a,b)) )  map { case(_,sql) => sql } mkString(",\n") )
      } map {
